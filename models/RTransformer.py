...
class LocalRNN(nn.Module):
    def __init__(self, input_dim, output_dim, rnn_type, ksize, dropout):
        ...
        # To speed up
        idx = [i for j in range(self.ksize-1, 10000, 1)
    ...
    def forward(self, x):
        ...
        # l 就是拼接之后有多少个 k_size
        # 其实就只有一个rnn，然后让 帧[0,1,2,3] 变成 [0,0,0, 0,0,1, 0,1,2, 1,2,3] 这个样子
        # [0,0,0]---rnn--> [0']   [0,0,1]---rnn-->[1'] ...
        # 然后通过rnn 变换得到 [0', 1', 2', 3']
        # [0]表示output  seq_len,batch,hidden  -1 表示 batch
...
class LocalRNNLayer(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, rnn_type, ksize, dropout):
        ...
        self.local_rnn = LocalRNN(
            input_dim, output_dim, rnn_type, ksize, dropout)
